...
gpx_end(const QString& el)
{
  ...
  case tt_garmin_rte_display_color:
    rte_head->line_color.bbggrr = gt_color_valucdatastr(cdatastr));
    break;
  case tt_rte_number:
  ...
  case tt_garmin_trk_display_color:
    trk_head->line_color.bbggrr = gt_color_valucdatastr(cdatastr));
    break;
  case tt_trk_number:
 ...
 e(void)
{
  ...
  if (gpx_wversion_num > 10) {
    writer->writeStartElement("metadataif (gpx_global) {
   ...
   */
  if (gpx_wversion_num < if (gpx_global) {
  ...
  writer->writeTextElement("time", now.toPrettyStriif (gpx_global) {
...
gpx_write_common_position(const Waypoint* waypointp, const gpx_point_type point_type)
{
  ...
  if (gpxpt_track==point_ == pe && 10==gpx_wversion_num) {
...
static void
gpx_exit(gpx_version.clear(); NULble? */
